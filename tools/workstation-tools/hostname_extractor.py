...
Small utility for extracting possible hostnames from copied text, logs,
ticket titles, or notes.

The script can read from:
- a text file
- pasted stdin
- a temporary Notepad window on Windows
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "file",
        nargs="?",
        help="Optional text file to read. If omitted, reads stdin or opens Notepad when --notepad is used.",
    )

    parser.add_argument(
        "--prefix",
        help="Only match hostnames that start with this prefix, for example LAB-PC-",
    )

    parser.add_argument(
        "--regex",
        help="Custom regular expression for hostname matching. Use carefully.",
    )

    parser.add_argument(
        "--output",
        "-o",
        help="Optional output text file for the extracted hostnames.",
    )

    parser.add_argument(
        "--uppercase",
        action="store_true",
        help="Print hostnames in uppercase. Default is lowercase.",
    )

    parser.add_argument(
        "--notepad",
        action="store_true",
        help="Open Notepad for pasted input. Windows only.",
    )
